...
# User Service Docker Management Script
# This script provides commands to build, run, test, and manage the Docker containerized User Service
...
IMAGE_NAME="scholar-user-service:latest"
...
run() {
    ...
    print_status "User Service is available at http://localhost:$DEFAULT_PORT"
    print_status "PostgreSQL is available at localhost:5433"
    ...
    print_success "User Service is running successfully!"
}
...
status() {
    if docker ps | grep -q "$CONTAINER_NAME"; then
        ...
        print_status "User Service: http://localhost:$DEFAULT_PORT"
        docker ps --filter "name=scholar-user" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
    else
...
show_help() {
    echo "User Service Docker Management Script"
    echo ""
